...
angular.module("vokal.datePicker", []).directive("datePicker", ["$compile", "$filter", "$document", "$timeout", function ($compile, $filter, $document, $timeout) {
    ...
    return {
        ...
        link: function link(scope, element, attrs, ngModelController) {
            ...
            var template = angular.element('<div class="date-picker" data-ng-show="showDatepicker">' + '<div class="month-name">{{ monthName }} {{ year }}</div>' + '<div class="month-prev" data-ng-click="buildMonth( prevYear, prevMonth )">&lt;</div>' + '<div class="month-next" data-ng-click="buildMonth( nextYear, nextMonth )">&gt;</div>' + '<div class="day-name-cell" data-ng-repeat="dayName in dayNames">{{ dayName }}</div>' + '<div class="filler-space" data-ng-repeat="space in filler"></div>' + '<div class="date-cell" ' + 'data-ng-class="{ today: dayNow == day && monthNow == month && yearNow == year }" ' + 'data-ng-repeat="day in days" data-ng-click="applyDate( month, day, year )">' + "{{ day }}</div></div>");
            ...
            var handler = function handler(event) {
                if (event.target !== element[0] && Array.prototype.slice.call(template[0].children).indexOf(event.target) === -1) {
